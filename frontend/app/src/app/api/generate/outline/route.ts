...
Generate a compelling, well-paced outline with ${chapter_count} chapters. Each chapter should advance the plot meaningfully.`
    ...
    console.log(`Generating outline for book ${book_id} with ${chapter_count} chapters...`)

    // Create generation log entry
    const logResult = await pool.query(
      `INSERT INTO generation_logs (book_id, generation_type, prompt, model, status)
       VALUES ($1, 'outline', $2, 'claude-sonnet-4-20250514', 'processing')
       RETURNING id`,
      [book_id, userPrompt]
    )
    ...
    try {
      ...
      await pool.query(
        ...
        ['Failed to parse AI response', JSON.stringify({ raw_response: aiText }), inputTokens, outputTokens, logId]
      )
    ...
    for (let i = 0; i < outline.chapters.length; i++) {
      ...
      const content = [
        ...
        chapter.summary,
        '',
        chapter.key_events?.length ? '### Key Events\n' + chapter.key_events.map(e => `- ${e}`).join('\n') : ''
      ].filter(Boolean).join('\n')
    ...
    console.log(`Outline generated: ${outline.chapters.length} chapters, ${inputTokens}/${outputTokens} tokens`)
