...
impl Prompts {
    ...
    pub fn chapter() -> PromptTemplate {
        PromptTemplate::new(
            "Generate a detailed outline for Chapter {chapter_number} of {total_chapters}, \"{chapter_title}\", maintaining narrative progression and story continuity.\n\nTitle: {title}\nBraindump: {braindump}\nGenre: {genre}\nStyle: {style}\nCharacters: {characters}\nSynopsis: {synopsis}\nBook Outline: {book_outline}\nThis chapter in the book outline: {chapter_brief}\nPrevious chapters: {previous_chapters}\nTemporary Summary: {temporary_summary}\n\nChapter Outline: Outline only Chapter {chapter_number} as planned above, continuing naturally from the previous chapters. Do not outline any other chapter and do not comment on the book's structure. Focus on character development, thematic elements, and maintaining the established tone.".to_string(),
            vec!["title".to_string(), "braindump".to_string(), "genre".to_string(), "style".to_string(), "characters".to_string(), "synopsis".to_string(), "book_outline".to_string(), "chapter_number".to_string(), "total_chapters".to_string(), "chapter_title".to_string(), "chapter_brief".to_string(), "previous_chapters".to_string(), "temporary_summary".to_string()],
            TemplateFormat::FString,
        )
    }
